...
class TestAccessibility:
    ...
    def test_bing_homepage_no_critical_violations(self, browser):
        ...
        if violations:
            ...
            print(f"Passes: {len(results.get('passes', []))}, Violations: {len(violations)}")
            for v in violations:
    ...
    def test_bing_search_form_accessible(self, browser):
        ...
        axe = Axe(browser)
        axe.inject()
        results = axe.run(context={"include": [["#sb_form"]]}, options={"runOnly": {"type": "tag", "values": ["wcag2a", "wcag2aa"]}})
        
        violations = results.get("violations", [])
        label_violations = [v for v in violations if "label" in v.get("id", "") or "aria" in v.get("id", "")]
    ...
    def test_saucedemo_login_form_accessible(self, browser):
        ...
        axe = Axe(browser)
        axe.inject()
        results = axe.run(context={"include": [["#login_button_container"]]}, options={"runOnly": {"type": "tag", "values": ["wcag2a", "wcag2aa"]}})
    ...
    def test_saucedemo_inventory_page_accessible(self, browser):
        ...
        axe = Axe(browser)
        axe.inject()
        results = axe.run(context={"include": [[".inventory_container"]]}, options={"runOnly": {"type": "tag", "values": ["wcag2a", "wcag2aa"]}})
