...
class TestPerformanceMonitor:
    ...
    def test_timer_decorator_default_name(self):
        ...
        assert_that(len(self.monitor.metrics), equal_to(1))
        assert_that(self.monitor.metrics[0].name, equal_to("sample_function_execution_time"))
...
class TestThreadSafety:
    ...
    def test_concurrent_metric_recording(self):
        ...
        def record_metrics(thread_id):
            for i in range(10):
                monitor.record_metric(f"thread_{thread_id}_metric_{i}", float(i), "units")
