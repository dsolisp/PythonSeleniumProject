...
class TestAPIContracts:
    ...
    def test_validate_films_endpoint_contract(self):
        ...
        assert_that(res.status_code, equal_to(200))
        body = res.json()
        keys = ['title', 'episode_id', 'opening_crawl', 'director', 'producer', 'release_date', 'created', 'edited', 'url']
        for k in keys:
            assert_that(body, has_key(k))
    ...
    def test_validate_planets_endpoint_contract(self):
        ...
        assert_that(res.status_code, equal_to(200))
        body = res.json()
        keys = ['name', 'rotation_period', 'orbital_period', 'diameter', 'climate', 'gravity', 'terrain', 'surface_water', 'population']
        for k in keys:
            assert_that(body, has_key(k))
    ...
    def test_ensure_contract_stability_no_unexpected_fields_removed(self):
        ...
        assert_that(res.status_code, equal_to(200))
        body = res.json()
        required_fields = ['name', 'height', 'mass', 'hair_color', 'skin_color', 'eye_color', 'birth_year', 'gender']
