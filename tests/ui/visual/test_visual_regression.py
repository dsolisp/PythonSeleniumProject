...
class TestSauceDemoVisual:
    ...
    def _assert_visual_snapshot(self, driver, snapshot_name, threshold=None):
        ...
        diff_pixels = diff_handler.compare_images(str(baseline_path), str(actual_path), str(diff_path))
        ...
        assert diff_pixels <= tol, f"Visual diff for {snapshot_name}: {diff_pixels} pixels differ"
    ...
    def test_minor_differences_10_percent_threshold(self, authenticated_driver):
        ...
        inventory_page.open()
        assert inventory_page.is_loaded()
        self._assert_visual_snapshot(authenticated_driver, 'inventory-flexible-comparison', threshold=0.10)

    def test_tiny_differences_1_percent_threshold(self, authenticated_driver):
        inventory_page = InventoryPage(authenticated_driver)
        inventory_page.open()
        assert inventory_page.is_loaded()
        self._assert_visual_snapshot(authenticated_driver, 'inventory-strict-comparison', threshold=0.01)
