"""
Enhanced API tests with better error handling and comprehensive validation.
"""
...
def test_create_and_retrieve_post():
    """Enhanced API test with comprehensive validation and error handling."""
    ...
    try:
        ...
        # Enhanced assertions for POST request
        assert_that(post_response.status_code, equal_to(201))
        assert post_response.headers.get('content-type'), "Response should have content-type header"
        ...
        # Enhanced assertions for GET request
        assert_that(get_response.status_code, equal_to(200))
        assert get_response.headers.get('content-type'), "Response should have content-type header"
...
def test_api_error_handling():
    ...
    try:
        ...
        invalid_response = requests.post(f"{base_url}/posts", json={"invalid": "data"}, timeout=10)
        # JSONPlaceholder is lenient, so it might return 201, but we test the structure
        assert invalid_response.status_code in [201, 400, 422], "Should handle invalid data appropriately"
