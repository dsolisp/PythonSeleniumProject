...
> **Scope:** Retry and failure formatting are implemented in **`utils/error_handler.py`** (`SmartErrorHandler.execute_with_retry`, `handle_error`, `ScreenshotService`, `format_error`). The snippets below match that **stdlib-only** design (no extra retry package in `requirements.txt`). Orchestration: `scripts/run_full_workflow.py` / `scripts/run_tests.py`.
