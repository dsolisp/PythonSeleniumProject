...
> **Recommended:** Use the integrated workflow script for all analytics and reporting. All analytics, reporting, and exports are now automatically triggered by running:

```bash
python run_full_workflow.py
```

This script runs all tests, exports results, performs analytics, generates dashboards, and enforces retention—no manual steps required.
...
Analytics and reporting are now fully automated as part of the unified workflow. The framework uses **pandas**, **numpy**, and **Jinja2** to transform raw test execution data into actionable insights, all orchestrated by `run_full_workflow.py`.
...
- **Performance analysis**: Identify slow tests and bottlenecks (see generated analytics CSV)
- **Trend tracking**: Monitor test suite health over time (see analytics summary)
- **Data export**: Results are exported for external tools (Excel, BI tools)
- **Executive reporting**: Dashboard-style reports are created for stakeholders
...
> ```
>
> All analytics, CSV exports, and dashboards are generated automatically in the `reports/` directory.
    </style>
</head>
<body>
    <h1>{{ suite_name }}</h1>
...
## 📊 Report Types (Generated by Workflow)
    </table>
</body>
</html>
''')
