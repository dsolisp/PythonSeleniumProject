...
Test Counts:
  ...
  regression  : Quick smoke test
    ...
    parser.add_argument(
        "--type",
        choices=["unit", "integration", "regression", "all", "api", "web", "performance"],
        default="regression",
    ...
    if args.type == "unit":
        ...
        for test_type, cmd, desc in [
            ("api", f"{base_cmd} tests/api/test_api.py", 
             "API Tests (5 tests)"),
            ("unit", f"{base_cmd} tests/unit/ --maxfail=10", 
             "Unit Tests (229 tests)"),
            ("integration", f"{base_cmd} tests/integration/", 
             "Integration Tests (19 tests)"),
            ("performance", f"{base_cmd} tests/performance/test_benchmarks.py", 
