...
class PerformanceMonitor:
    ...
    def set_threshold(self, metric_name: str, threshold_value: float, unit: str = "ms") -> None:
    ...
    def record_metric(self, name: str, value: float, unit: str = "ms", **context) -> None:
    ...
    def measure_cpu_usage(self, interval: float = 0.1, name: str = "cpu_usage") -> float:
        """Measure CPU usage and record as metric."""
        cpu_percent = psutil.cpu_percent(interval=interval)
        self.record_metric(name, cpu_percent, "%")
        return cpu_percent
    
    def benchmark_function(self, func: Callable, iterations: int = 100, *args, **kwargs) -> Dict[str, float]:
...
class APIPerformanceMonitor(PerformanceMonitor):
    ...
    def monitor_api_request(self, session, method: str, url: str, **kwargs) -> Dict[str, float]:
        ...
        # Record metrics
        self.record_metric("api_response_time", response_time, "ms", 
                          method=method, url=url, status_code=response.status_code)
        self.record_metric("api_total_time", total_time, "ms",
                          method=method, url=url, status_code=response.status_code)
        ...
        def wrapper(*args, **kwargs):
            ...
            if threshold_ms and execution_time > threshold_ms:
                raise AssertionError(
                    f"Performance threshold exceeded: {execution_time:.2f}ms > {threshold_ms}ms"
                )
