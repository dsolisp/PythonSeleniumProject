...
class AdvancedTestReporter:
    ...
    def __init__(self, reports_dir: Union[str, Path] = None):
        ...
        self.reports_dir = Path(reports_dir) if reports_dir else Path(__file__).parent.parent / "reports"
    ...
    def generate_json_report(self, filename: str = None) -> str:
        ...
        pass_rate = (self.current_suite.passed / self.current_suite.total_tests * 100) if self.current_suite.total_tests > 0 else 0
        avg_duration = self.current_suite.total_duration / self.current_suite.total_tests if self.current_suite.total_tests > 0 else 0
    ...
    def generate_trend_analysis(self, days: int = 30) -> Dict[str, Any]:
        ...
        # Save trend analysis
        trend_file = self.reports_dir / "trends" / f"trend_analysis_{datetime.now().strftime('%Y%m%d')}.json"
    ...
    def _analyze_performance(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "duration_std_dev": round(statistics.stdev(durations) if len(durations) > 1 else 0, 2),
    ...
    def _generate_recommendations(self, trends: List[TestTrend]) -> List[str]:
        ...
        # Analyze pass rates
        recent_pass_rates = [t.pass_rate for t in trends[-7:]]  # Last 7 executions
        ...
        if avg_duration > 15:
            recommendations.append("Optimize test execution time - average duration > 15s")
        if avg_duration > 30:
...
<body>
    ...
    <div class="test-results">
        ...
        <table>
            ...
            {''.join([f'<tr><td>{r.test_name}</td><td class="status-{r.status.lower()}">{r.status}</td><td>{r.duration:.2f}s</td><td>{r.error_message or ""}</td></tr>' for r in self.test_results])}
        </table>
    </div>
</body>
</html>
...
<body>
    ...
    <div class="dashboard">
        ...
        <div class="widget">
            ...
            <ul>
                {''.join([f'<li>{failure}</li>' for failure in trend_data.get('most_common_failures', [])])}
            </ul>
        </div>
    </div>
</body>
    ...
    def _generate_junit_xml(self) -> str:
        ...
        total_time = self.current_suite.total_duration
        test_cases = []
        
        for result in self.test_results:
            case_xml = f'    <testcase name="{result.test_name}" time="{result.duration}"'
...
<testsuite name="{self.current_suite.suite_name}" 
           ...
           failures="{self.current_suite.failed}" 
           errors="{self.current_suite.errors}" 
           skipped="{self.current_suite.skipped}" 
           time="{total_time}">
{''.join(test_cases)}
</testsuite>"""
