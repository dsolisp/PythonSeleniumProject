...
class MLTestAnalyzer:
    ...
    def detect_flaky_tests(self, min_runs: int = 3) -> pd.DataFrame:
        ...
        flaky_analysis = self.df.groupby('test_name').agg({
            'status': ['count', lambda x: (x == 'passed').sum()],
            'duration': 'mean'
        }).reset_index()
        
        flaky_analysis.columns = ['test_name', 'total_runs', 'passed_runs', 'avg_duration']
    ...
    def analyze_performance_trends(self) -> Dict[str, Any]:
        ...
        z_scores = np.abs((self.df['duration'] - stats['avg_duration']) / stats['std_duration'])
    ...
    def get_test_statistics(self) -> pd.DataFrame:
        ...
        stats.columns = [
            'test_name', 'total_runs', 'passed', 'failed',
            'avg_duration', 'min_duration', 'max_duration', 'std_duration'
        ]
        
        stats['pass_rate'] = stats['passed'] / stats['total_runs']
        stats['reliability_score'] = stats['pass_rate'] * (1 - stats['std_duration'] / stats['avg_duration'].clip(lower=0.01))
    ...
    def train_failure_predictor(self) -> Tuple[float, Any]:
        ...
        feature_cols = [col for col in df_ml.columns if col.endswith('_encoded')] + ['duration']
    ...
    def predict_test_failures(self, upcoming_tests: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        ...
        for test in upcoming_tests:
            ...
            predictions.append({
                ...
                'recommendation': 'High risk - run first' if prob > 0.7 else 
